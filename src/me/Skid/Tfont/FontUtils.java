...
public class FontUtils {
    ...
    private Font getFont(String fontName, int fontType, int size) {
        ...
        try {
            InputStream inputstream = FontManager.class.getResourceAsStream("/assets/minecraft/loserline/fonts/" + fontName);
    ...
    public void drawLimitedStringWithAlpha(String text, float x, float y, int color, float alpha, float maxWidth) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (c != 49 && (index == 0 || index == characters.length - 1 || characters[index - 1] != 49)) {
                if (index >= 1 && characters[index - 1] == 49) {
                    continue;
                }
            ...
            } else if (c == 49 && index != characters.length - 1) {
    ...
    private String processString(String text) {
        ...
        text = str.replace("ï¿½ï¿½r", "").replace('â–?', '=').replace('â?', 'â™?').replace('â‹?', 'â˜?').replace('â˜?', 'â˜?').replace('âœ?', 'â˜?').replace("âœ?", "â˜?").replace("âœ?", "+");
    ...
    public String trimStringToWidth(String text, int width, boolean reverse) {
        ...
        for (int k = i; k >= 0 && k < text.length() && f < (float) width; k += j) {
            ...
            if (flag) {
                ...
                f += (float) ((double) f2 / (text.contains("=====") ? 2.2D : 2.0D));
                if (flag2) {
                    ++f;
                }
            }
