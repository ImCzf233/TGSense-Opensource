...
public class GlyphPageFontRenderer {
    ...
    public GlyphPageFontRenderer(GlyphPage regularGlyphPage, GlyphPage boldGlyphPage, GlyphPage italicGlyphPage, GlyphPage boldItalicGlyphPage) {
        ...
        for (int i = 0; i < 32; ++i) {
            ...
            this.colorCode[i] = (k & 255) << 16 | (l & 255) << 8 | i2 & 255;
        }
    ...
    private void renderStringAtPos(String text, boolean shadow) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (c0 == 49 && i + 1 < text.length()) {
                int i = "0123456789abcdefklmnor".indexOf(text.toLowerCase(Locale.ENGLISH).charAt(i + 1));
    ...
    private void doDraw(float f, GlyphPage glyphPage) {
        ITessellator tessellator2;
        IWorldRenderer worldrenderer2;

        if (this.strikethroughStyle) {
            tessellator2 = (ITessellator) Tessellator.getInstance();
            worldrenderer2 = tessellator2.getWorldRenderer();
            GlStateManager.disableTexture2D();
            worldrenderer2.begin(7, (IVertexFormat) DefaultVertexFormats.POSITION);
            worldrenderer2.pos((double) this.posX, (double) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2)), 0.0D).endVertex();
            worldrenderer2.pos((double) (this.posX + f), (double) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2)), 0.0D).endVertex();
            worldrenderer2.pos((double) (this.posX + f), (double) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F), 0.0D).endVertex();
            worldrenderer2.pos((double) this.posX, (double) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F), 0.0D).endVertex();
            tessellator2.draw();
            GlStateManager.enableTexture2D();
        }
        ...
        if (this.underlineStyle) {
            ...
            worldrenderer2.pos((double) (this.posX + (float) l), (double) (this.posY + (float) glyphPage.getMaxFontHeight()), 0.0D).endVertex();
            worldrenderer2.pos((double) (this.posX + f), (double) (this.posY + (float) glyphPage.getMaxFontHeight()), 0.0D).endVertex();
            worldrenderer2.pos((double) (this.posX + f), (double) (this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F), 0.0D).endVertex();
            worldrenderer2.pos((double) (this.posX + (float) l), (double) (this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F), 0.0D).endVertex();
            tessellator2.draw();
            GlStateManager.enableTexture2D();
        }
    ...
    public int getStringWidth(String text) {
        if (text == null) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                if (character == 49) {
                    on = true;
                } else if (on && character >= 48 && character <= 114) {
                    int i = "0123456789abcdefklmnor".indexOf(character);
